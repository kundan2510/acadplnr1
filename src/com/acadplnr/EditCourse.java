...
public class EditCourse extends Activity implements OnClickListener {
	Global1 globe;
	EditText courseNm, courseLnk, instructname, off_hr_add, pemail;
	Button bLab, bLec, bTut, bdone;
	String lec_st, lec_et, tut_st, hdata, welink, tut_et, lab_st, lab_et,
			lec_v, tut_v, lab_v, lec_d, lab_d, tut_d, c_name, w_link, ins_name,
			off_hradd, email;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		pemail = (EditText) findViewById(R.id.email_id);
	...
	public void onClick(View arg0) {
		...
		case R.id.bdoneeditadd:
			...
			info.open();
			try {
				info.createEntry(c_name, ins_name, w_link, off_hradd, lec_d,
						lec_st, lec_et, tut_d, tut_st, tut_et, lab_d, lab_st,
						lab_et, lec_v, lab_v, tut_v, hdata, email);
				info.KeyRowIdUpdate();
			} catch (Exception e) {
				String error = e.toString();
				Dialog h = new Dialog(this);
